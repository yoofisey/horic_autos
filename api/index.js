...
function genId() { return 'v' + Date.now().toString(36) + crypto.randomBytes(3).toString('hex'); }
function genEnqId() { return 'e' + Date.now().toString(36) + crypto.randomBytes(3).toString('hex'); }
function generateToken(admin) { return jwt.sign({ id: admin.id, email: admin.email }, JWT_SECRET, { expiresIn: '7d' }); }
...
async function generateEmbedding(text) {
  ...
  } catch (err) { console.error('Embedding error:', err.message); return null; }
}
...
async function embedAndStore(text, contentType, metadata) {
  ...
  const [data] = await sql`INSERT INTO knowledge_base (content, content_type, metadata, embedding) VALUES (${text}, ${contentType}, ${metaJson}::jsonb, ${embeddingStr}::vector) RETURNING id, content, content_type, metadata, created_at`;
  return data;
}
...
async function ragSearch(query, matchCount = 5, filterType = null) {
  ...
  let results;
  if (filterType) {
    results = await sql`SELECT id, content, content_type, metadata, 1 - (embedding <=> ${embeddingStr}::vector) as similarity FROM knowledge_base WHERE content_type = ${filterType} AND 1 - (embedding <=> ${embeddingStr}::vector) > ${threshold} ORDER BY embedding <=> ${embeddingStr}::vector LIMIT ${matchCount}`;
  } else {
    results = await sql`SELECT id, content, content_type, metadata, 1 - (embedding <=> ${embeddingStr}::vector) as similarity FROM knowledge_base WHERE 1 - (embedding <=> ${embeddingStr}::vector) > ${threshold} ORDER BY embedding <=> ${embeddingStr}::vector LIMIT ${matchCount}`;
  }
  return results || [];
}
...
app.post('/api/vehicles', requireAuth, async (req, res) => {
  try {
    ...
    const [vehicle] = await sql`INSERT INTO vehicles (id, make, model, trim, year, price, condition, status, body_type, fuel, mileage, engine, transmission, color, description, features, images, created_at, updated_at, views, enquiries)
      VALUES (${id}, ${v.make || ''}, ${v.model || ''}, ${v.trim || ''}, ${Number(v.year) || 2024}, ${Number(v.price) || 0}, ${v.condition || 'new'}, ${v.status || 'in_stock'}, ${v.body_type || 'sedan'}, ${v.fuel || 'petrol'}, ${Number(v.mileage) || 0}, ${v.engine || ''}, ${v.transmission || 'automatic'}, ${v.color || ''}, ${v.description || ''}, ${featuresJson}::jsonb, ${imagesJson}::jsonb, ${now}::timestamptz, ${now}::timestamptz, 0, 0) RETURNING *`;
...
app.put('/api/vehicles/:id', requireAuth, async (req, res) => {
  try {
    ...
    const featuresJson = JSON.stringify(v.features || []);
    const imagesJson = JSON.stringify(v.images || []);
    const [vehicle] = await sql`UPDATE vehicles SET make = ${v.make}, model = ${v.model}, trim = ${v.trim || ''}, year = ${Number(v.year)}, price = ${Number(v.price)}, condition = ${v.condition}, status = ${v.status}, body_type = ${v.body_type}, fuel = ${v.fuel}, mileage = ${Number(v.mileage)}, engine = ${v.engine}, transmission = ${v.transmission}, color = ${v.color}, description = ${v.description}, features = ${featuresJson}::jsonb, images = ${imagesJson}::jsonb, sold_price = ${v.sold_price ? Number(v.sold_price) : null}, sold_date = ${v.sold_date || null}, sold_to = ${v.sold_to || null}, updated_at = ${now}::timestamptz WHERE id = ${req.params.id} RETURNING *`;
    const kbText = vehicleToKnowledge(vehicle);
...
app.post('/api/enquiries', async (req, res) => {
  try {
    const id = genEnqId();
    const e = req.body;
    const [enquiry] = await sql`INSERT INTO enquiries (id, vehicle_id, customer_name, customer_phone, customer_email, message, status, created_at) VALUES (${id}, ${e.vehicle_id || null}, ${e.customer_name || 'Anonymous'}, ${e.customer_phone || ''}, ${e.customer_email || ''}, ${e.message || ''}, 'unread', now()) RETURNING *`;
    res.json(enquiry);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/enquiries/:id', requireAuth, async (req, res) => {
  try {
    const e = req.body;
    const [enquiry] = await sql`UPDATE enquiries SET status = ${e.status}, customer_name = ${e.customer_name}, message = ${e.message} WHERE id = ${req.params.id} RETURNING *`;
    res.json(enquiry);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/stats', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ totalCars: cars.length, inStock: inStock.length, sold: sold.length, totalValue, totalEnquiries: enqs.length, unreadEnquiries: enqs.filter(e => e.status === 'unread').length, knowledgeEntries: kbRow?.count || 0 });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const response = await openai.chat.completions.create({ model: 'gpt-4o-mini', messages: chatMessages, max_tokens: 1024, temperature: 0.7 });
