const HoricAdmin = (() => {
  ...
  function renderDashboard() {
    ...
    document.getElementById('statValue').textContent = HoricData.formatPrice(stats.totalValue);
  ...
  function renderDashboardTable(mode) {
    var tbody = document.getElementById('dashTableBody');
    if (!tbody) return;
    var cars = HoricData.loadInventory();
    if (mode === 'popular') {
      cars.sort(function(a, b) { return (b.views || 0) - (a.views || 0); });
    } else {
      cars.sort(function(a, b) { return new Date(b.created_at) - new Date(a.created_at); });
    }
    cars = cars.slice(0, 8);
    tbody.innerHTML = cars.map(function(car) {
      return '<tr>' +
        ...
        '<td>' + HoricData.formatPrice(car.price) + '</td>' +
        '<td>' + (car.views || 0) + '</td>' +
  ...
  function renderSales() {
    var cars = HoricData.loadInventory().filter(function(c) { return c.status === 'sold'; });
    var totalRevenue = cars.reduce(function(s, c) { return s + (c.sold_price || c.price); }, 0);
    document.getElementById('salesTotalSold').textContent = cars.length;
    document.getElementById('salesRevenue').textContent = HoricData.formatPrice(totalRevenue);
    document.getElementById('salesAvgPrice').textContent = cars.length ? HoricData.formatPrice(Math.round(totalRevenue / cars.length)) : 'GHS 0';
    ...
    tbody.innerHTML = cars.map(function(car) {
      return '<tr>' +
        '<td><div class="table-car-info"><div class="table-car-thumb">' + CAR_SVG + '</div>' +
        '<div><div class="table-car-name">' + car.make + ' ' + car.model + '</div><div class="table-car-sub">' + car.year + '</div></div></div></td>' +
        '<td style="font-weight:600;color:var(--green-700);">' + HoricData.formatPrice(car.sold_price || car.price) + '</td>' +
        '<td>' + (car.sold_to || '—') + '</td>' +
        '<td>' + (car.sold_date || '—') + '</td>' +
  ...
  function renderEnquiries() {
    var enquiries = HoricData.loadEnquiries();
    ...
    list.innerHTML = enquiries.map(function(enq) {
      var car = enq.vehicle_id ? HoricData.getVehicle(enq.vehicle_id) : null;
      return '<div class="enquiry-card ' + (enq.status === 'unread' ? 'unread' : '') + '">' +
        ...
        (car ? '<div class="enquiry-vehicle">Re: ' + car.year + ' ' + car.make + ' ' + car.model + ' — ' + HoricData.formatPrice(car.price) + '</div>' : '') +
  ...
  function markEnquiryRead(id) {
    HoricData.updateEnquiry(id, { status: 'read' });
    renderEnquiries();
    var stats = HoricData.getStats();
    document.getElementById('enquiryBadge').textContent = stats.unreadEnquiries;
    document.getElementById('statUnread').textContent = stats.unreadEnquiries;
  ...
  function editVehicle(id) {
    var car = HoricData.getVehicle(id);
    if (!car) return;
  ...
  function saveVehicle(e) {
    ...
    if (id) {
      HoricData.updateVehicle(id, data);
      toast('Vehicle updated successfully', 'success');
    } else {
      HoricData.addVehicle(data);
  ...
  function openSoldModal(id) {
    ...
    document.getElementById('soldPrice').value = car.price;
  ...
  function confirmSold() {
    ...
    HoricData.markAsSold(id, price, to);
  ...
  function relistVehicle(id) {
    HoricData.updateVehicle(id, { status: 'in_stock', sold_price: null, sold_date: null, sold_to: null });
    renderSales();
    renderDashboard();
