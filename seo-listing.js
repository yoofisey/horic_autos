...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
      ...
      if (!list.length) {
        el.innerHTML = '<div class="no-results" style="grid-column:1/-1;"><div class="no-results-icon">!</div><h3>No vehicles right now</h3><p>Our inventory changes often. Check the full inventory or ask AI Assistant for the latest arrivals.</p><div class="no-results-actions"><a class="btn btn-primary btn-sm" href="inventory.html">Browse Full Inventory</a><button class="btn btn-outline btn-sm" onclick="document.querySelector(\'.chat-toggle\').click()">Ask AI Assistant</button></div></div>';
        return;
      }
      el.innerHTML = list.map(buildCard).join('');
    } catch (e) {
      el.innerHTML = '<div class="no-results" style="grid-column:1/-1;"><div class="no-results-icon">!</div><h3>Could not load vehicles</h3><p>Please try again or contact us directly.</p><div class="no-results-actions"><a class="btn btn-primary btn-sm" href="inventory.html">Browse Full Inventory</a><a class="btn btn-outline btn-sm" href="https://wa.me/233000000000">WhatsApp Us</a></div></div>';
    }
  });
})();
